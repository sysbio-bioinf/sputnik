...
(ns feature-selection-jppf.main
  (:gen-class)
  (:require
    ...
    [feature-selection-jppf.fitness :as f]
    [feature-selection-jppf.remote :as r]
    [feature-selection-jppf.ga :as ga]
...
(defn+opts run-classification-feature-selection
  ...
  (let [[dataset-fn, load-fn, file-spec] [(if pre-defined-folds `ds/csv-dataset-sequential `ds/csv-dataset-by-class),
        ...
        ^org.jppf.client.JPPFClient client (when (= mode :distributed)
                                             (if (nil? client-config)
                                               ...
                                               (scr/create-client-from-config client-config)))
...
(defn- print-help
  ...
  ([banner, msg]
    ...
    (println
      (format
        (str "Usage: java -jar feature-selection-jppf-<VERSION>.jar run <OPTIONS> <FILES>\n\n"
          "Options:\n"
          "%s")
        banner))
    (flush)))
    ...
    (println
      (format
        (str "Feature Selection\n"
          "Usage: java -jar feature-selection-jppf-<VERSION>.jar [-h] [setup|run] <OPTIONS> <FILES>\n\n"
          "Modes:\n"
          "- Setup server and workers for distributed computation:\n"
          "    java -jar feature-selection-jppf-<VERSION>.jar setup\n"
          "- Run the feature selection with the given options and files:\n"
          "    java -jar feature-selection-jppf-<VERSION>.jar run <OPTIONS> <FILES>\n\n"
          "Options:\n"
          "%s")          
        banner))
      (flush)))
