...
(ns feature-selection-jppf.remote
  (:require
    ...
    [jppfnik-client.protocol :as scp]
    ...
    (loop [id 0, task-coll (transient [])]
      (if (< id n)
        (let [batch-data (nth batches id),
              task (scp/create-task id,
                     'feature-selection-jppf.remote/batch-evaluation,
    ...
    (mapv
      (fn [i, task]
        (cond-> task
          ...
          (scp/conj-task-data sleep-duration)))
      (range)
      tasks)
    tasks))
...
(defn distributed-evaluation
  ...
  (let [{:keys [tasks, individuals-map]} (prepare-remote-evaluation-tasks-batched sample-list, load-datasets-fn, file-list, population, batch-size),
        ...
        job (scp/create-job (format "%s - %03d" (fmt/datetime-filename-format (java.lang.System/currentTimeMillis)), job-id), tasks),
;        _ (log/debugf "Starting computation of job %s:\n%s" (:job-id job) (with-out-str (pp/pprint job))),
        begin-time (System/currentTimeMillis),
        results (scr/compute-job client, job),
  ...
  (let [individuals (mapv
    ...
    (loop [id 0, task-coll (transient [])]
      (if (< id n)
        (let [batch-data (nth batches id),
              task (scp/create-task id,
                     'feature-selection-jppf.remote/batch-test,
...
(defn distributed-test
  ...
  (let [{:keys [tasks, individuals-map]} (prepare-remote-test-tasks-batched train-instance-coords, test-instance-coords, load-datasets-fn, file-list, population, batch-size),
        job (scp/create-job (format "%s - test" (fmt/datetime-filename-format (java.lang.System/currentTimeMillis))), tasks),
;        _ (log/debugf "Starting computation of job %s:\n%s" (:job-id job) (with-out-str (pp/pprint job)))
        begin-time (System/currentTimeMillis),
        results (scr/compute-job client, job),
        end-time (System/currentTimeMillis)]
