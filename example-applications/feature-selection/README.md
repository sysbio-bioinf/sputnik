...
TODO: Beschreibung wie man ein fertig gebautes Release bekommt.
...
```bash
$ java -jar target/feature-selection-1.0.0-SNAPSHOT.jar setup
```
...
```bash
$ java -jar target/feature-selection-1.0.0.jar run -h
...
```bash
$ java -jar target/feature-selection-1.0.0.jar run data/west.csv --folds data/west_folds.txt \
...
```bash
$ java -jar target/feature-selection-1.0.0.jar run data/<DATASET>.csv --folds data/<DATASET>_folds.txt \
--one-probability 0.01 --flip-one-probability 0.5 --flip-zero-probability 0.5 --crossover-probability 0.01 \
